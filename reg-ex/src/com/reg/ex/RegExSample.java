...
public class RegExSample {
	...
	public static String extractQuotationText(String quotation) {
		...
		String questionValue = getQuestionValue(questionFormat1,quotation,new Integer[]{0,1,2},2);
	 ...
	 * @return
	 */
	private static String getQuestionValue(final Pattern ptn, final String str,Integer[] fetch,Integer grp) {
		String questionValue = null;
		final Matcher matcher = ptn.matcher(str);
		if (matcher.find()) {
			questionValue = matcher.group(grp);
			for(int index : fetch){
				System.out.println(" matcher.group() "+index+" "+matcher.group(index));
			}
